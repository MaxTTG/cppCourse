...
void processGroup(std::vector<FileReader *> &readers, size_t blockIndex, size_t blockSize, size_t totalBlocks, std::vector<std::vector<FileReader *> > &duplicates) {
    if (readers.size() < 2) {
        return;
    }
...
boost::regex wildcardToRegex(const std::string &mask, bool caseInsensitive) {
    std::string regexStr;
...
bool fileMatchesConditions(const boost::filesystem::path &filePath, const Options &opts, uint64_t fsize) {
...
std::vector<FileInfo> scanDirectoryNonRecursive(const boost::filesystem::path &dirPath, const Options &opts) {
...
std::vector<FileInfo> scanDirectoryRecursive(const boost::filesystem::path &dirPath, const Options &opts) {
    std::vector<FileInfo> files;
